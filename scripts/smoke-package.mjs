...
const hubEntry = path.join(repoRoot, "dist/hub/hub.cjs");
const agentEntry = path.join(repoRoot, "dist/agent/agent.cjs");
const cwd = await mkdtemp(path.join(tmpdir(), "pa-pkg-cwd "));

const handshake = await run(process.execPath, [hubEntry], {
  cwd,
  env: {
    ...process.env,
...
if (handshake.stdout.includes("[hub]") || handshake.stdout.includes("[agent]")) {
  fail("stdout del Hub empaquetado contiene logs (deben ir a stderr)", handshake.stdout);
}
if (handshake.code !== 0) {
  fail("handshake empaquetado falló", handshake.stderr);
}
if (!handshake.stderr.includes("Agent READY")) {
  fail("no hubo Agent READY", handshake.stderr);
}
...
if (pidMatch) {
  ...
  try {
    ...
    fail(`Agent pid ${agentPid} sigue vivo tras shutdown`);
  } catch {
    /* expected: ESRCH */
  }
}
...
const sdkRoot = path.join(
  repoRoot,
  "hub/node_modules/@modelcontextprotocol/sdk",
);
