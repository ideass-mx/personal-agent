...
## Principio rector
Nada se conecta directo a nada: **todo pasa por el Agent API (`api/`)**.
Los dispositivos son puntos de entrada/salida; los nodos ejecutores hacen;
el api piensa y enruta.

## Piezas

**Agent API (`api/` · Node/TS)** — el cerebro / runtime local. Cuatro trabajos:
...
SALIENTE al api (nunca puertos abiertos). `Core/` (gateway, cola de tareas,
...
(ejecuta y notifica), destructivo (confirmación por voz vía api). Audit log.

**Protocolos** — dos capas, no mezclar: clientes↔api = WebSocket JSON propio
(packages/protocol); api↔nodos ejecutores = MCP.

**Red** — Tailscale entre dispositivos durante desarrollo. El api corre en la
PC hoy; portable a VPS sin tocar nada más (los agentes cambian una URL).

## Decisiones selladas (con fecha y razón)

- Agent API en Node/TS (ecosistema MCP/agentes) tras evaluar Spring Boot.
- Monorepo plano en raíz (api/, android/, agent-windows/) — sin carpeta apps/
...
- Wear OS: **backlog**, no camino crítico. Diseño listo: módulo `wear` hermano
  ...
  patrón Android; peaje Siri vía App Intents; push APNs para api→usuario.
