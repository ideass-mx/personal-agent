...
const PORT = path.join(repoRoot, "gateway/src/agents/confirmation.ts");
const WAITER = path.join(repoRoot, "gateway/src/sessions/confirmation-waiter.ts");
const MEMORY = path.join(repoRoot, "gateway/src/memory/types.ts");
const SQLITE = path.join(repoRoot, "gateway/src/memory/sqlite-turn-memory.ts");
