...
const NODE_MAIN = path.join(repoRoot, "node/src/index.ts");
...
describe("PHASE 25 Node / MCP topology (audit)", () => {
  ...
  it("composición: spawn único; Runtime y Node sin cruzar fronteras", () => {
    ...
    assert.ok(index.indexOf("attachLocalNode") < index.indexOf("[gateway] READY"));
    const shutdownFn = index.slice(index.indexOf("const shutdown"));
    assert.ok(
      shutdownFn.indexOf("http.close") < shutdownFn.indexOf("localNode.shutdown"),
    );
