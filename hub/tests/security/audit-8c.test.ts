...
} from "../../../agent/src/tools/process-execute.ts";
import {
  createAgentRuntime,
  type AgentEvent,
  type TurnMemory,
...
describe("8C arquitectura Hub ↔ Agent", () => {
  ...
  it("read/write/list comparten resolveSafePath", () => {
    for (const name of [
      ...
      const src = readFileSync(
        path.join(repoRoot, "agent/src/tools", name),
        "utf8",
      );
  ...
  it("process.execute spawn: shell false y stdio no hereda MCP stdout", () => {
    const src = readFileSync(
      path.join(repoRoot, "agent/src/tools/process-execute.ts"),
      "utf8",
    );
  ...
  it("no hay tercer proceso en el pipeline Hub/Agent", () => {
    for (const file of [
      ...walkTs(path.join(repoRoot, "hub/src")),
      ...walkTs(path.join(repoRoot, "agent/src")),
    ]) {
      const text = readFileSync(file, "utf8");
      assert.doesNotMatch(text, /from ["'].*guardian/, file);
    }
