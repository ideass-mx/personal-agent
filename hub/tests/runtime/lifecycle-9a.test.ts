...
const hubRoot = path.join(repoRoot, "hub");
...
describe("9A lifecycle Hub → Agent", () => {
  it("A–D: spawn, initialize, tools/list, READY solo con Agent", async () => {
    ...
    const readyAt = index.indexOf('[hub] READY');
  ...
  it("K/L: stdout del Agent no trae banners; stderr sí logs", async () => {
    const agentPkg = path.join(repoRoot, "agent");
    const child = spawn(
      process.execPath,
      [
    ...
    try {
      await waitUntil(() => stderr.includes("[agent] MCP stdio listo"), 15_000);
      await new Promise((r) => setTimeout(r, 200));
      assert.equal(stdout.includes("[agent]"), false);
      assert.equal(stdout.includes("listo"), false);
      assert.match(stderr, /\[agent\] MCP stdio listo/);
    } finally {
      child.kill("SIGTERM");
