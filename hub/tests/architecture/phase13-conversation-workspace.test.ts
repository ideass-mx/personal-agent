...
const TURN = path.join(repoRoot, "hub/src/memory/types.ts");
const SESSIONS = path.join(repoRoot, "hub/src/http/sessions.ts");
const NODE_CFG = path.join(repoRoot, "agent/src/config.ts");
const MCP_STDIO = path.join(repoRoot, "hub/src/tools/mcp-stdio.ts");
const PHASE13 = path.join(
  repoRoot,
  "docs/architecture/phase13-conversation-workspace.md",
);
const FORBIDDEN =
  /\b(CapabilityRegistry|WorkspaceManager|ContextManager|ConversationWorkspaceManager|AgentRegistry|NodeRegistry|ActiveWorkspace|WorkspaceRegistry)\b/;
