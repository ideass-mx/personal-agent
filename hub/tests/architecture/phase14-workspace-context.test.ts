...
const RESOLVE = path.join(
  repoRoot,
  "hub/src/memory/resolve-workspace-for-conversation.ts",
);
const STORE = path.join(
  repoRoot,
  "hub/src/workspace/sqlite-workspace-store.ts",
);
const TYPES = path.join(repoRoot, "hub/src/workspace/types.ts");
...
const FORBIDDEN =
  /\b(ActiveWorkspace|activeWorkspaceId|WorkspaceManager|WorkspaceRegistry|WorkspaceRouter|ContextManager|AgentRegistry|NodeRegistry|CapabilityRegistry)\b/;
