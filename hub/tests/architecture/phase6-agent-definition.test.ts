...
const NODE_CONFIG = path.join(repoRoot, "agent/src/config.ts");
...
describe("PHASE 6 Agent Definition", () => {
  it("una sola implementación de Agent Runtime", () => {
    const runtimes = walkTs(path.join(repoRoot, "hub/src")).filter(
      (f) => path.basename(f) === "runtime.ts" && f.includes(`${path.sep}agent${path.sep}`),
    );
  ...
  it("Agent Definition no ejecuta y no importa infraestructura", () => {
    ...
    assert.doesNotMatch(src, /\bworkspaceId\b|\bconversationId\b/);
    assert.match(
      src,
      /configuration-scoped and not yet persistent/,
    );
  });
