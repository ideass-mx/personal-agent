...
const GATEWAY_CFG = path.join(repoRoot, "hub/src/config.ts");
...
describe("PHASE 8 Agent / Node configuration", () => {
  it("AgentDefinition es lógica: sin root, credenciales, MCP, ids", () => {
    ...
    assert.doesNotMatch(src, /\bnodeId\b|\bworkspaceId\b|\bagentId\b/);
  ...
  it("un Runtime; producción MCP; sin registries/Capability/A2A/Workspace store", () => {
    ...
    assert.equal(
      walkTs(path.join(repoRoot, "hub/src/agent")).filter(
    ...
    assert.match(
      readFileSync(path.join(repoRoot, "hub/src/index.ts"), "utf8"),
      /attachLocalAgent/,
    );
  });
