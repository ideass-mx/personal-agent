...
const HISTORY = path.join(repoRoot, "hub/src/memory/history.ts");
const SESSIONS = path.join(repoRoot, "hub/src/http/sessions.ts");
const HTTP = path.join(repoRoot, "hub/src/http/workspace-http.ts");
const TOOLS = path.join(repoRoot, "hub/src/tools/types.ts");
const PROTO = path.join(repoRoot, "packages/protocol/messages.ts");
const SCHEMA = path.join(repoRoot, "db/schema.sql");
const FORBIDDEN =
  /\b(ActiveWorkspace|activeWorkspaceId|WorkspaceManager|ConversationManager|ConversationContext|AgentRegistry|NodeRegistry)\b/;
